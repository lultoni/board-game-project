#!/usr/bin/env python3
"""Migrate docs/design-principles.md → principles table."""
...
# Hand-curated extraction: each row = (id, kind, n, title, status, body, established_in)
# Bodies are the relevant prose extracted verbatim (markdown preserved).

rows = []

# Core Fantasy — treat as a north-star companion
rows.append((
    "principle-core-fantasy", "north-star", None, "Core Fantasy",
    "active",
...
rows.append((
    "principle-high-concept-framing", "lens", None, "High-Concept Framing — Two minds, one puzzle",
    "active",
...
This is design intent, not a mechanical mandate. No immediate rule changes follow from it. See ADR-004 in the `adrs` table for the alternatives considered and the reversal criterion.""",
    "session-20",
))

# Chassis and Engine lens
rows.append((
    "principle-chassis-and-engine", "lens", None, "Chassis and Engine",
    "active",
...
# North Star
rows.append((
    "principle-north-star", "north-star", None, "North Star",
    "active",
...
# The Five Principles (Session 7, ADR-003)
five = [
...
for n, title, body in five:
    rows.append((f"principle-{n}", "principle", n, title, "active", f"*Established Session 7 (ADR-003).*\n\n{body}", "session-7"))

# Principle 6 (Session 23)
rows.append((
    "principle-6", "principle", 6, "Game length is itself a form of attrition.",
    "active",
...
# Principle 7 (Session 23) — conditional
rows.append((
    "principle-7", "principle", 7, "While the core identity is unsettled, prefer fundamental shifts over variable tweaking.",
    "active",
    """*Established Session 23 (2026-05-30).*
...
# Hard Constraints
hard = [
    ("perfect-info", "Perfect information — no dice, no hidden cards, no randomness.",
     "All information is open. No dice, no hidden cards, no randomness. Source: Session 1, designer mandate.", "session-1"),
    ("grid-based", "Grid-based — spatial positioning stays.",
     "The board makes spells interesting. Source: Session 1 (ADR-001/002).", "session-1"),
    ("no-terrain", "No terrain effects — board is uniform.",
     "Confirmed overhead complexity. Board is uniform. Source: Session 1 (ADR-001/002).", "session-1"),
    ("two-skill-slots", "2 skill slots per Champion/King.",
     "Forces specialist builds, creates draft tension. 3 slots risks generalist meta. Source: Session 8 analysis.", "session-8"),
    ("blank-champions", "Champions are blank slates.",
     "Identity emerges from equipped skills, not pre-naming. Mental freedom to do anything. Source: Session 10, designer feedback.", "session-10"),
    ("guards-late-game", "Guards have late-game presence.",
     "Guards stay at 2 HP. Design should not force a \"kill Guards first\" sequence. Source: Session 7, designer mandate.", "session-7"),
]
for slug, title, body, sess in hard:
    rows.append((f"constraint-{slug}", "hard-constraint", None, title, "active", body, sess))

# Spending Tension (G8)
rows.append((
    "principle-spending-tension", "principle", None, "Spending Tension (G8)",
    "active",
...
# Economy Philosophy
rows.append((
    "principle-economy-philosophy", "principle", None, "Economy Philosophy",
    "active",
...
# Incremental Testing Methodology (encoded as principle/lens — keep as principle)
rows.append((
    "principle-incremental-testing", "principle", None, "Incremental Testing Methodology",
    "active",
...
Conditional override: Principle 7 (Session 23) suspends Incremental Testing while the core identity is unsettled — bundled stacks are allowed when justified.""",
    "session-1",
))

# Cognitive Load
rows.append((
    "principle-cognitive-load", "lens", None, "Cognitive Load (3–5 decision variables)",
    "active",
...
for r in rows:
    cur.execute(
        "INSERT INTO principles (id, kind, n, title, status, body, established_in) VALUES (?, ?, ?, ?, ?, ?, ?)",
        r,
    )
conn.commit()

print(f"Inserted {len(rows)} principle rows:")
for r in rows:
    print(f"  {r[0]:<38} kind={r[1]:<16} title={r[3][:50]}")

cur.execute("SELECT COUNT(*) FROM principles;")
print(f"\nTotal in principles: {cur.fetchone()[0]}")
conn.close()
