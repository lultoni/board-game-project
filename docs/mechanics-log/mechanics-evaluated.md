...
| Armor cap 3→2 + Armorsmith +1→+2 (bundled) | OQ-11 / Q-C1 | Stack H | Reduces chassis-loop volume (Framing B alignment); Armorsmith becomes one-shot fortify rather than stack-grind. Bundled — coupling documented. Gated on Stack A G2 (need confirmation that combo bonus prevents Armor stalling). Rollback variant: cap 3→2 only (C1a) as Stack I if stalling emerges. |
| Injured: remove mechanical downsides (no speed cap, no Range −1) | OQ-57 / Q-B5 | Stack J | Tests whether Injured's chassis volume (speed cap, Range −1, Range-modifier interactions, self/adjacent carve-out) pays for itself in game-feel terms. State still exists as HP-tracker. Gated behind Stack A G2 + Stack H. Could scale up to baseline-change candidate if it plays well. |
