...
*Last updated: 2026-05-30 — Session 23 (close).*
...
1. **OQ-61 (new — two-pole framing)** — Pole A vs Pole B as parallel design tracks; experiment-that-could-replace per current user lean. Resolution: after first 2–3 Pole B prototype games, compare game-feel vs Pole A.
2. **OQ-11 (Queued; Stack H deprioritised)** — bundled dose remains the lead variant when Stack H runs. "Build cheaper than break" risk noted as bigger than originally framed. Re-evaluated after Pole B prototype data lands.
3. **OQ-62 (new — Pole A draft determinism)** — sequential draft drives "always better to react" pathology. Simultaneous-reveal proposal accepts limited PI loss in pre-game window only.
4. **OQ-63 (new — cross-pole fixing methodology)** — when a problem exists in both poles, test fixes once or per pole? Resolved on first encounter; user lean: twice for cleanness.
...
**Run the first Pole B per-turn-draft prototype game digitally** (3-week vacation window with Jonathan opens). No physical rule sheet for the first 2–3 prototype runs. After 2–3 games, compare game-feel vs Pole A and route per Stack L's *Routing on result* in TESTING_PLAN.
